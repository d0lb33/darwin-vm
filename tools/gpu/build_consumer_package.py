...
subprocess.run(['xcrun','clang',*flags,f'-DDVM_CA_FRAMES={a.frames}',f'-DDVM_CA_HZ={a.hz}',f'-DDVM_CA_SCENE={a.scene}','-c',str(src/'consumer_package.m'),'-o',str(a.out/'consumer_package.o')],check=True)
...
record=dict(frames=a.frames,hz=a.hz,scene=a.scene,binary_sha256=hashlib.sha256(binary.read_bytes()).hexdigest(),
...
for name in ('consumer_package.m','consumer_probe.inc','consumer_sequence_probe.inc'):
