...
def verify_records(out,lines,records,expected_frames=1,expected_scene=0):
    ...
    if expected_scene not in range(4) or (expected_scene and not sequence):raise ValueError('scene contract')
    ...
    if sequence:
        ...
        for frame,read in zip(checkpoints,reads):
            ...
            if len(pixels)!=len(expected) or any(abs(a-b)>(1 if expected_scene==1 and i%4!=3 else 0) for i,(a,b) in enumerate(zip(pixels,expected))):raise ValueError('host target differs from moving layer oracle')
    else:
    ...
    scene_names=('moving-opaque-layers','half-opacity-layer','clipped-scaled-layer','image-layer')
