...
def main():
    p=argparse.ArgumentParser();p.add_argument('out',type=Path);p.add_argument('--stubs',required=True,type=Path);a=p.parse_args()
    a.out.mkdir(exist_ok=False);shutil.copytree(a.stubs,a.out/'stubs')
    ...
    run(['python3',str(repo/'tools/gpu/verify_guest_imports.py'),'--output',str(a.out/'object-providers.tsv'),str(object_imports)])
    groups={}
    ...
    run(['python3',str(repo/'tools/gpu/verify_guest_imports.py'),'--output',str(a.out/'imports.tsv'),str(imports)])
