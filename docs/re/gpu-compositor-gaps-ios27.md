...
| P0: accelerated scanout format/color | Exact QuartzCore sRGB tags at `0x18452c018`; IOSurface transfer table `0x1bffbf8dc`; EDR enable at A408+0x54c | RGHA_GUEST2 has four actual RGhA presentations and subsequent D594 completions, alternating two imported surfaces | Scoped finite sRGB RGhA→SDR BGRA conversion; packed PPM capture; ordered completion matching even with reused ID zero | Native CoreGraphics half-pattern oracle; exact final source→conversion→console byte match | Full DCP matrices/curves/HDR, independent compositor semantic oracle, sustained display/input and native alias retirement |
...
| P0: compute descriptors and mixed command order | QuartzCore descriptor builder `0x1845abce0` sets function/label and gates thread limit/archives; direct call `0x1844f3828` | RGHA_GUEST3 creates compute_average_luma then fails newComputePipelineStateWithDescriptor:error: before dispatch | Existing function-based audited luma/blur compute; render/blit ordered path; descriptor and mixed compute/render batch still missing | Earlier exact compute workloads, not this compositor call | Actual descriptor options/reflection, command combinations and broader compute reflection/dispatch contracts |
