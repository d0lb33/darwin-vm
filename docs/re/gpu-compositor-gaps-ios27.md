...
| P0: compute descriptors and mixed command order | Builder `0x1845abce0`, creation `0x1844f3828`; explicit serial encoder type zero at `0x1844f44ac` | COMPUTE_GUEST4 RPC434 creates actual compute_average_luma pipeline, then stops at a texture view before dispatch | V25 owned specialized functions, native reflection validation, serial render/compute/blit batches and writable-buffer completion delivery | Six native/forwarded mixed frames including 64 commands; three exact-AIR host reduction batches; actual guest pipeline creation; malformed commands reject before execution | Exact compositor dispatch remains unverified; active stage input, linking, guest reflection, concurrent/nonuniform/tile dispatch unsupported |
| P0: private screen intermediates | Allocation geometry derives from actual descriptor | COMPUTE_GUEST2 requests 1216×2560 RGBA16F private usage 65541; old 16 MiB bound rejects | V24 private cap 32 MiB, ordinary allocation cap 64 MiB, copied/transfer/import limits unchanged | Eight host forwarded frames at this geometry across four mips, changing edge samples, exact pixels and complete retirement; allocation/budget rejection tests | Exact new allocation/dispatch and sustained growth still need guest validation |
| P0: single-mip texture views | QC `0x1844eb8cc` preserves source format/type, selects one level beginning at x20 and slice {0,1}; reductions follow at `0x1844eb964/eba2c` | COMPUTE_GUEST4 throws on this selector in actual DVMTexture after pipeline creation | Unimplemented | None | Runtime source/mip selection unknown; aliases need ownership, purgeability, transfer and subresource hazard contracts |
