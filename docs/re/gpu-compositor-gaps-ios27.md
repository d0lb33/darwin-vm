...
| P0: accelerated scanout format/color | QEMU `darwin_iomfb_swap.c:77` accepts only BGRA | PURGEABILITY_GUEST1 submits A408; parser rejects primary BGRA profile and withholds D594; unique stopped record contains fullscreen RGhA/row9472 | Existing BGRA display path preserved; explicit failure now stops the bounded test | Earlier owned-pool display acceptance; new raw surface/candidate swap captured, colors not independently verified | RGhA scanout/color metadata and conversion, DCP completion and sustained display/input |
...
Correct pixels and native display ownership remain acceptance gates.
