...
| P0: purgeability and discard/reacquire | Seven direct QuartzCore calls to Volatile/NonVolatile; exact FlattenManager pool call `0x18471da04`; other receiver classes unresolved | PURGEABILITY_GUEST1 request 256 transitions owned 256 KiB buffer 35 from 2 to 3 successfully | V21 native owned-resource transitions, alias state, dirty-write retention, Empty cache invalidation and uncertain-ack quarantine | Host native/shared/private/alias/refill and lost-ack CPU/GPU rejection; exact guest first Volatile request | Exact guest reacquisition/Empty; pinned IOSurface volatility remains explicitly unsupported; recovery after uncertain reply deferred |
| P0: accelerated scanout format/color | Exact QuartzCore sRGB tags at `0x18452c018`; IOSurface transfer table `0x1bffbf8dc`; EDR enable at A408+0x54c | COMPUTE_GUEST1 has eight actual RGhA presentations and subsequent D594 completions, alternating two imported surfaces | Scoped finite sRGB RGhA→SDR BGRA conversion; packed PPM capture; ordered completion matching even with reused ID zero | Native CoreGraphics half-pattern oracle; exact final source→conversion→console byte match; COMPUTE_GUEST1 two Home edges and subsequent display with stable input helper | Full DCP matrices/curves/HDR, independent compositor semantic oracle, sustained pacing, full input semantics and native alias retirement |
| P0: copied texture and object-table budgets | Supported bytes/objects derive from transport bounds, not host hardware limits | RGHA_GUEST1 requests 1280×932 RG8 (2,385,920 bytes); RGHA_GUEST2 passes byte validation then hits 128 live objects, including 42 distinct functions/39 distinct pipelines | V22 4 MiB ordinary images, 32 KiB chunks, 1 MiB legacy reads, 256 live objects; total byte budgets preserved | Native/forwarded six-frame large-image comparisons, chunk/GPU-copy/bounds and object-table release/reuse; RGHA_GUEST3 allocates/uploads the actual RG8 image and uses it in completed pass 10 | Long-running cache growth and sustained memory use remain unverified |
| P0: compute descriptors and mixed command order | Builder `0x1845abce0`, creation `0x1844f3828`; serial encoder type zero at `0x1844f44ac` | TEXTURE_VIEW_GUEST1 RPC330 executes actual average/sum luma, with 18 render passes and two dispatches | Owned specialized functions, native reflection validation, serial render/compute/blit batches | Actual guest GPU completion; 64 displayed batches overall; exact-AIR host view reductions and malformed-command rejection | Independent guest luma-output oracle, sustained pacing; active stage input/linking, guest reflection and concurrent/nonuniform/tile dispatch unsupported |
| P0: simultaneous private intermediates | Allocation geometry derives from actual descriptors | TEXTURE_VIEW_PACING1 creates 1216×2560 private target209; RPC972 requests a second fullscreen usage65541 intermediate and exceeds 64 MiB aggregate cap | Per-image 32 MiB and aggregate 64 MiB ordinary budget; import/copy limits separate | Host screen/mip pixels and budget rejection; exact successful first large allocation, clean rejection of second | Account for simultaneous working set and release/reuse before increasing aggregate contract; sustained growth unverified |
| P0: single-mip texture views | QC `0x1844eb8cc` preserves source format/type and selects one level | TEXTURE_VIEW_GUEST1 RPC325: parent155, RGBA16F 576×64 private, mip0/count1, slice0/count1; view169 used by compute | V26 same-format 2D mip ranges, nested ownership, native residency metadata and alias validation | Exact guest view creation/use; host 24-frame pixels, imported-view retirement/lost-reply tests, overlapping-blit rejection | Guest final-alias retirement, reinterpretation/arrays/planes/swizzles and broader concurrency |
| P0: ordered command capacity | Count is an explicit transport bound | COMPUTE_GUEST3 submits 35 render passes/575 operations and exceeds old 32-pass limit | V25 negotiated 64 commands, 2 MiB staging retained | Native/forwarded 64-command batch; 65-command preflight rejection | Exact 35/64-pass compositor execution and sustained pacing not yet verified |
...
alignment/submission fixes and GUEST23’s 21 passes/58 draws.
[V21 evidence](gpu-resource-purgeability-ios27.md) adds the actual owned-buffer
Volatile transition and identifies the next explicit RGhA scanout rejection.
[RGhA scanout evidence](gpu-rgha-compositor-ios27.md) supersedes that format
rejection: four presentations/completions and exact final console delivery now
pass. Compositor semantic correctness, full native ownership and sustained
pacing remain acceptance gates.
...
[Descriptor-compute evidence](gpu-compute-descriptor-ios27.md) records the current coherent batch and its host/guest distinction. The eight-frame boot passes native Home dispatch and later display; it is not sustained pacing or system-wide Liquid Glass acceptance.

Wallpaper lead: `lock-screen-wallpaper.md` identifies MercuryPosterExt as a separate Metal consumer. Backboardd-only registration does not supply that process a device. Verify its actual factory result and selected API path before extending registration; its static Metal 4 references are not proof of a required runtime path.

Latest V28 result: [development profile / actual Home transition](gpu-development-profile-ios27.md)
clears both former capacity gates in CA_DEV_HOME1. 493 completed GPU/display
batches, Home recovery and final delivery pixels pass. A visible 648 ms
transition gap remains; missing wallpaper and complete scene correctness remain
