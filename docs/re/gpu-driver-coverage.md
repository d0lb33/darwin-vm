...
| Iteration | Fresh-process supervisor, signed package staging, incremental build and captured-submission replay tools | Two exact guest fresh-process GPU/pixel controls; 27-request host replay passes; Data/signature/remount failures captured | Installed driver passed; live revision trust and Data mapping unresolved; System remount denied |
...
1. **Persistent staging/loading:** boot one isolated runner, require existing
   ...
   a newly signed revision through controlled staging paths. Data-volume tests have already failed: a boot-trusted bundle reports `file system sandbox blocked mmap()`; an absent-from-boot-trust-cache revision reports `code signature invalid`. The helper-only `com.apple.private.security.no-sandbox` entitlement did not fix the tested Data path. Two unchanged System-installed controls passed. System remount returned EPERM/77 before staging. The scoped kernel service plus helper `get-task-allow` now obtains stock TXM approval, but both Data loading failures remain. Four normal GPU controls passed before/after the development jobs. Next: instrument the exact file-signature/mapping callbacks; see [kernel evidence](gpu-development-loader-ios27.md). Each job
