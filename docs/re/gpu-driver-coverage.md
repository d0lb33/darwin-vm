...
| Resources | Dirty buffer spans, partial 2D/3D transfers, 6 texture formats (A8 sampled only), read-only linear views, private GPU textures (including read/render block-write RGBA8/BGRA8) and opaque guest process metadata | Exact 2D transfer/control and private intermediate allocation; host GPU dependency, linear/3D transfers, private CPU-access rejection | General IOSurface import/planes, heaps, memoryless storage, private blit transfers; texture axes 4096, private images 16 MiB, copied images 1 MiB |
...
| UIKit effects | Actual UIVisualEffectView material/glass probes; original guest QuartzCore shaders | Host material blur matches native in all 3 frames; exact guest blur executes 9 passes/23 draws and validated replay reproduces pixels | Exact guest effect pixel reference pending; no-window glass has no effect; displayed effects/system integration unproven. See [effect evidence](gpu-uikit-effects-ios27.md) |
...
3. **UIKit content:** exact guest job 1788775984328876 passes the full 320×480
   ...
   Material blur now executes through the exact guest driver; its independent effect pixel oracle and normal-window glass remain unresolved. See [effect contracts and staging](gpu-uikit-effects-ios27.md). Next: real window context and captured filter inputs. System
