...
## 4. Measured: the AP answers a callback, but only after a second message
...
The mechanism is visible but not settled. That second message takes
...
Nothing in the disassembly read so far distinguishes them, so the model keeps
it behind `DARWIN_DCP_IOMFB_CB_KICK`, default off, rather than pretending to
know. Settling it is the next piece of work: the `[slot+0x50]` `vtable+0xf8`
call at `0xa0cfcdc` — the one that emits the acknowledgement — is the place to
start, because whatever object that is also owns the `+0x108` wake.

## 5. Sweep: 23 zero-input callbacks, all accepted, none resumes the AP
...
**The negative result matters more than the positive one.** After all 23, the
